...
生の`.xlsx`を読めない場合、番号付きPromptの開始前に、人間が同じシートをTSV、CSV、Markdown表、または行・節ID付きテキストへ書き出します。書き出した同一版を入力スナップショットとして全後続工程へ渡し、推測で続行しません。この前処理は`/00`〜`/10`の工程には含めません。

## 2. 1回の処理単位

- `E2J-00`: 原則1ブック
- `E2J-01`〜`E2J-06`: 原則1シートまたは1論理ブロック

...
Office OOXMLファイルをMarkdownとassetディレクトリへ変換するVS Code拡張の成果物一式です。
...
- 画像と埋め込みオブジェクトは `*.assets/` に抽出し、Markdownから相対リンクします。
...
3. Explorerで `.xlsx`, `.xlsm`, `.pptx`, `.docx` を右クリックします。
4. `Convert Office File to Markdown` を実行します。

Command Paletteからは `Office Markdown: Convert Active Office File to Markdown` を実行できます。
...
- OCR、LLM Vision、Officeページの画像レンダリング、複雑なSmartArt/Chart再構成は対象外です。
